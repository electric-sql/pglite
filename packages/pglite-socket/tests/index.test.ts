...
describe('PGLiteSocketHandler', () => {
  ...
  let mockSocket: ReturnType<typeof createMockSocket> & { eventHandlers: Record<string, Array<(data: any) => void>> }
  ...
  it('should reject attaching multiple sockets', async () => {
    ...
    await expect(handler.attach(anotherMockSocket)).rejects.toThrow('Socket already attached')
  })
