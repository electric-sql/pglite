...
test.each([
  {
    profile: 'generic dispatch',
    transformed: 'opcodes.multi.wasm',
    report: 'report.json',
  },
  {
    profile: 'inline private fast path',
    transformed: 'opcodes.inline.wasm',
    report: 'report.inline.json',
  },
])(
  'preserves runtime semantics with $profile',
  async ({ transformed: transformedName, report: reportName }) => {
    const originalBytes = await readFile(artifact('opcodes.wasm'))
    const transformedBytes = await readFile(artifact(transformedName))
    const report = JSON.parse(await readFile(artifact(reportName), 'utf8'))
    ...
    const original = (await instantiateOriginal(originalMemory)).instance
      .exports
    const transformed = (
    ...
    for (const name of exportedNames.filter((name) =>
      ...
      const args = name.includes('cmpxchg')
    ...
    for (let i = 0; i < 2000; i++) {
      ...
      if (random() & 1) {
        ...
        const size =
          1 + (random() % Math.min(32, 4096 - Math.max(address, source)))
    ...
    await new Promise((resolve, reject) => {
      const worker = new Worker(
        new URL('./workers/transformed-runtime.mjs', import.meta.url),
        {
